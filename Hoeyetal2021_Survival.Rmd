...
p2 = ggplot(p2, aes(y = -.value, x = Site)) +
  stat_eye(alpha=0.7, position = position_dodge(width = 0.7)) + 
  theme_bw(base_size = 12) + geom_hline(yintercept=0, linetype="dashed") +
  # scale_y_continuous(breaks = c(0.25, 0.5,0.75,1,1.25), limits = c(0.25,1.5)) +
...
egg::ggarrange(p1, p2, ncol=2, widths = c(1.2,1))
...
mod.brms.bites = brm(formula =TotalBites ~ bite_on*habitat +site  + (1|id/Date), 
...
mod.brms.bites2 = brm(formula =TotalBites ~ bite_on*habitat*site  + (1|id/Date), 
                    data=dat.bites.sum, family="negbinomial", sample_prior = T,
                    # prior = m1priors,
                    prior = c(prior(normal(0, 5), class = "Intercept"),
                              prior(normal(0, 5), class = "b"),
...
mod.brms.bites4 = brm(formula =TotalBites ~ bite_on*habitat + site + (1|Date), 
                    data=dat.bites.sum, family="negbinomial", sample_prior = T,
                    # prior = m1priors,
                    prior = c(prior(normal(0, 5), class = "Intercept"),
                              prior(normal(0, 5), class = "b"),
...
mod.brms.bites5 = brm(formula =TotalBites ~ bite_on*habitat+site*habitat + (1|Date) + (1|id), 
                    data=dat.bites.sum, family="negbinomial", sample_prior = T,
                    # prior = m1priors,
                    prior = c(prior(normal(0, 5), class = "Intercept"),
                              prior(normal(0, 5), class = "b"),
...
mod.brms.bites6 = brm(formula =TotalBites ~ bite_on*habitat*site + (1|id), 
                    data=dat.bites.sum, family="negbinomial", sample_prior = T,
                    # prior = m1priors,
                    prior = c(prior(normal(0, 5), class = "Intercept"),
                              prior(normal(0, 5), class = "b"),
...
mod.brm2 = add_criterion(mod.brms.bites2,  "waic")
mod.brm3 = add_criterion(mod.brms.bites3,  "waic")
mod.brm4 = add_criterion(mod.brms.bites4,  "waic")
mod.brm5 = add_criterion(mod.brms.bites5,  "waic")
mod.brm6 = add_criterion(mod.brms.bites6,  "waic")
loo::loo_compare(mod.brm1, mod.brm2, mod.brm3, mod.brm4,mod.brm5,mod.brm6,criterion = "waic")

mod_em = emmeans::emmeans(mod.brms.bites,~ bite_on*habitat + site)
mod_em
summary(mod_em, type = "response")
multcomp::cld(mod_em)
# perf.brms1 = performance::check_model(mod.brms.surv)
...
p5 = dat.bites.sum %>%
  ...
  add_fitted_draws(mod.brms.bites, re_formula = NULL,  n=100) 
p4 = dat.bites.sum %>%
  modelr::data_grid(bite_on, habitat, site) %>%
  # spread_draws()
  add_fitted_draws(mod.brms.bites, re_formula = NULL, n=100)  %>%
  compare_levels(.value, by = bite_on) %>%
  mutate(ifelse(.value <0,0.001,.value))

ggplot(p5, aes(y = .value/3, x = habitat, fill = habitat)) +
  theme_bw(base_size = 12) + 
  coord_cartesian(ylim= c(0,600)) +
  geom_point(data=dat.bites.sum, aes(y=TotalBites/3, x=habitat, colour=habitat), position = position_jitterdodge(),pch=1) +
...
p3.1 = ggplot(p3, aes(y = .value/3, x = site, fill = bite_on)) +
  theme_bw(base_size = 12) + 
  # coord_cartesian(ylim= c(0,400)) +
  geom_point(data=dat.bites.sum, aes(y=TotalBites/3, x=site, colour=bite_on), position = position_jitterdodge(),pch=1) +
  stat_eye(alpha=0.7, position = position_dodge(width = 0.7), .width = c(0.5, 0.95)) +
  # stat_gradientinterval(position = "dodge") +
  # scale_y_continuous(breaks = c(0.25, 0.5,0.75,1,1.25), limits = c(0.25,1.5)) +
  scale_fill_viridis_d() +
...
egg::ggarrange(p3.1, p4.1, ncol=2, widths = c(1.2,1))
```
...
pCS.1 = ggplot(pCS1, aes(y = (.value/3), x=habitat, fill = bite_on, shape=bite_on)) +
  theme_bw(base_size = 12) +
  stat_eye(alpha=0.7, position = position_dodge(width = 0.7), .width = c(0.5, 0.95)) +
  coord_cartesian(ylim= c(0,20)) +
  geom_point(data=dat.bites.spmodCS, aes(y=(TotalBites/3), x=habitat, colour=bite_on), 
...
pSC.1 = ggplot(pSC1, aes(y = (.value), x=habitat, fill = bite_on, shape=bite_on)) +
  theme_bw(base_size = 12) +
  stat_eye(alpha=0.7, position = position_dodge(width = 0.7), .width = c(0.5, 0.95)) +
...
pAC.1 = ggplot(pAC1, aes(y = (.value), x=habitat, fill = bite_on, shape=bite_on)) +
  theme_bw(base_size = 12) +
  stat_eye(alpha=0.7, position = position_dodge(width = 0.7), .width = c(0.5, 0.95)) +
...
pES.1 = ggplot(pES1, aes(y = (.value), x=habitat, fill = bite_on, shape=bite_on)) +
  theme_bw(base_size = 12) +
  stat_eye(alpha=0.7, position = position_dodge(width = 0.7), .width = c(0.5, 0.95)) +
  coord_cartesian(ylim= c(0,400)) +
...
pPO.1 = ggplot(pPO1, aes(y = (.value), x=habitat, fill = bite_on, shape=bite_on)) +
  theme_bw(base_size = 12) +
  stat_eye(alpha=0.7, position = position_dodge(width = 0.7), .width = c(0.5, 0.95)) +
...
pPO.2 = ggplot(pPO2, aes(x = .value, y = habitat)) +
  ...
  # xlab("Difference in Bites (Turf-Propagules)")
egg::ggarrange(pCS.1, pCS.2,pSC.1, pSC.2, pAC.1,pAC.2,ncol=2, widths = c(1.2,1))
egg::ggarrange(pES.1, pES.2,pPO.1, pPO.2,
               pSC.1, pSC.2, pAC.1,pAC.2,pCS.1, pCS.2,ncol=2,nrow = 5, widths = c(1.2,1),heights = c(1.5, 1.5,1,1,1))
```
