...
m2priors = get_prior(formula =proportion.remaining ~ Treatment*Habitat + (1|Site), 
                    data=dat.prop, family="gamma")
mod.brms.surv2 = brm(formula =proportion.remaining ~ Treatment*Habitat + (1|Site), 
                    data=dat.prop, family="gamma", sample_prior = T,
                    prior = c(prior(normal(0, 5), class = "Intercept"),
                              prior(normal(0, 5), class = "b"), 
                              prior(gamma(0.1, 0.1), class = "shape")),
                    warmup = 400, iter = 10000, chains = 3, cores = 3, thin=2
                    # control = list(adapt_delta = 0.995,
                    #                max_treedepth=15)
                    )

m3priors = get_prior(formula =proportion.remaining ~ Treatment*Habitat, 
                    data=dat.prop, family="gamma")
mod.brms.surv3 = brm(formula =proportion.remaining ~ Treatment*Habitat, 
                    data=dat.prop, family="gamma", sample_prior = T,
                    prior = m3priors,
                    warmup = 400, iter = 10000, chains = 3, cores = 3, thin=2
                    # control = list(adapt_delta = 0.995,
...
```
```{r Part 1.2 1.3 - Propagule Survival Pooled + Random factor parametrisation}
m1.2priors = get_prior(formula =proportion.remaining ~ Treatment*Habitat, 
                    data=dat.prop, family="gamma")
m1.3priors = get_prior(formula =proportion.remaining ~ Treatment*Habitat + (1/Site), 
                    data=dat.prop, family="gamma")
mod.brms.surv2 = brm(formula =proportion.remaining ~ Treatment*Habitat, 
                    data=dat.prop, family="gamma", sample_prior = T,
                    prior = m1.2priors,
                    warmup = 200, iter = 2000, chains = 3, cores = 3, thin=2
                    # control = list(adapt_delta = 0.995,
...
mod.brms.surv3 = brm(formula =proportion.remaining ~ Treatment*Habitat +  (1/Site), 
                    data=dat.prop, family="gamma", sample_prior = T,
                    prior = c(prior(normal(0, 5), class = "Intercept"),
                              prior(normal(0, 5), class = "b"), 
                              prior(gamma(0.1, 0.1), class = "shape")),
...
ggsave("Figures/PropSurvivalPooled.svg", plot = g, device = "svg", 
       width = 8, height = 5, dpi=300)
emmeans::emmeans(mod.brms.surv2,~ Treatment*Habitat, type="response")
pairs(emmeans::emmeans(mod.brms.surv2,~ Treatment*Habitat, type="response"))
#Random Factor Sites----
summary(mod.brms.surv2)
p1.3 = dat.prop %>%
  modelr::data_grid(Treatment, Habitat) %>%
  add_fitted_draws(mod.brms.surv3, re_formula = NULL, scale = "response") 
p2.3 = dat.prop %>%
  modelr::data_grid(Treatment, Habitat) %>%
  add_fitted_draws(mod.brms.surv3, re_formula = NULL, scale = "response") %>%
  compare_levels(.value, by = Treatment) 
p1.3 = ggplot(p1.3, aes(y = .value, x = Treatment, fill = Treatment)) +
  theme_bw(base_size = 12) + 
  geom_point(data=dat.prop, aes(y=proportion.remaining, x=Treatment, colour=Treatment), 
             position = position_jitterdodge(),pch=1) +
  stat_eye(alpha=0.7, position = position_dodge(width = 0.7)) + 
...
p2.3 = ggplot(p2.3, aes(x = -.value, y = Habitat)) +
  stat_pointinterval(alpha=0.7, position = position_dodge(width = 0.7)) + 
  theme_bw(base_size = 12) + geom_vline(xintercept=0, linetype="dashed") +
  # scale_y_continuous(breaks = c(0.25, 0.5,0.75,1,1.25), limits = c(0.25,1.5)) +
  scale_y_discrete(limits=rev) +
  ...
  # facet_wrap(~Habitat, ncol = 1, scales = "free_y") +
  xlab("Difference in proportion of propagules \nremaining (Caged-Exposed)") 
g=egg::ggarrange(p1.3, p2.3, ncol=2, widths = c(1.4,1))
ggsave("Figures/PropSurvivalRandom.png", plot = g, device = "png", 
       width = 8, height = 5, dpi=300)
ggsave("Figures/PropSurvivalRandom.pdf", plot = g, device = cairo_pdf, 
       width = 8, height = 5, dpi=300)
ggsave("Figures/PropSurvivalRandom.svg", plot = g, device = "svg", 
       width = 8, height = 5, dpi=300)
emmeans::emmeans(mod.brms.surv3,~ Treatment*Habitat, type="response")
```
...
mod.brms.bites4 = brm(formula =TotalBites ~ bite_on*habitat + (1|Date), 
                    data=dat.bites.sum, family="negbinomial", sample_prior = T,
                    # prior = m1priors,
                    prior = c(prior(normal(0, 5), class = "Intercept"),
                              prior(normal(0, 5), class = "b"),
...
mod.brm1 = add_criterion(mod.brms.bites,  "loo")
mod.brm3 = add_criterion(mod.brms.bites3,  "loo")
mod.brm4 = add_criterion(mod.brms.bites4,  "loo")

loo::loo_compare(mod.brm1,  mod.brm3,mod.brm4, criterion = "loo")
...
p3b = dat.bites.sum %>% as.data.frame() %>%
  modelr::data_grid(bite_on, habitat, site) %>% 
  add_fitted_draws(mod.brms.bites3, re_formula = NULL,  n=1000) %>% 
  mutate(habitat = factor(ifelse(habitat == "Reef crest", "Crest", "Flat"), 
                           levels = c("Crest", "Flat")))

# p3c = dat.bites.sum %>% as.data.frame() %>%
#   modelr::data_grid(bite_on, habitat, Date) %>% 
#   add_epred_draws(mod.brms.bites4, re_formula = NULL,  n=1000) %>% 
#   mutate(habitat = factor(ifelse(habitat == "Reef crest", "Crest", "Flat"), 
#                            levels = c("Crest", "Flat")))
    
p3c = mod.brms.bites4 %>% 
  emmeans(~ bite_on | habitat, epred = TRUE, re_formula = NA) %>% 
...
p3b = mod.brms.bites3 %>% emmeans(~ bite_on|habitat + site, type="response") %>%
  gather_emmeans_draws(type="response") %>%
  mutate(.value = exp(.value))# %>% 
  mutate(habitat = factor(ifelse(habitat == "Reef crest", "Crest", "Flat"), 
                           levels = c("Crest", "Flat"))) 

p4b = mod.brms.bites3 %>% emmeans(~ bite_on|habitat + site, type="response") %>%
  gather_emmeans_draws(type="response") %>%
  mutate(.value = exp(.value)) %>%
  compare_levels(.value, by = bite_on) %>%
  mutate(habitat = factor(ifelse(habitat == "Reef crest", "Crest", "Flat"), 
...
```{r Part 2.2 - Grazing Pooled and Random Parametrisation}
...
ggplot(dat.bites.sum, aes(x=site, y=TotalBites, fill = bite_on)) +
  geom_boxplot() + facet_wrap(~habitat)

# mod.brms.bites.1 = brm(formula =TotalBites ~ bite_on*habitat + (1|id/Date/site), 
#                     data=dat.bites.sum, family="negbinomial", sample_prior = T,
#                     # prior = m1priors,
#                     prior = c(prior(normal(0, 5), class = "Intercept"),
#                               prior(normal(0, 5), class = "b"),
...
mod.brms.bites.2 = brm(formula =TotalBites ~ bite_on*habitat  + (1|id/Date), 
                    data=dat.bites.sum, family="negbinomial", sample_prior = T,
                    # prior = m1priors,
                    prior = c(prior(normal(0, 5), class = "Intercept"),
                              prior(normal(0, 5), class = "b"),
...
mod.brms.bites.2.1 = brm(formula =TotalBites ~ bite_on*habitat  + (1|Date) + (1|id), 
                    data=dat.bites.sum, family="negbinomial", sample_prior = T,
                    # prior = m1priors,
                    prior = c(prior(normal(0, 5), class = "Intercept"),
                              prior(normal(0, 5), class = "b"),
...
mod.brms.bites.2ZI = brm(formula =TotalBites ~ bite_on*habitat  + (1|Date/site), 
                    data=dat.bites.sum, family=zero_inflated_negbinomial(), sample_prior = T,
                    # prior = m1priors,
                    prior = c(prior(normal(0, 5), class = "Intercept"),
                              prior(normal(0, 5), class = "b"),
                              prior(student_t(3, 0, 2.5), class = "sd"),
                              prior(gamma(0.1, 0.1), class = "shape")),
                    warmup = 400, iter = 4000, chains = 3, cores = 3, thin=2,
                    control = list(adapt_delta = 0.995)
                    #                max_treedepth=15)
                    )
...
mod.brms.bites.6 = brm(formula =TotalBites ~ bite_on*habitat + (habitat|site), 
                    data=dat.bites.sum, family="negbinomial", sample_prior = T,
                    # prior = m1priors,
                    prior = c(prior(normal(0, 5), class = "Intercept"),
                              prior(normal(0, 5), class = "b"),
...
mod.brms.bites.7 = brm(formula =TotalBites ~ bite_on*habitat +site*habitat + (1|Date), 
                    data=dat.bites.sum, family="negbinomial", sample_prior = T,
                    # prior = m1priors,
                    prior = c(prior(normal(0, 5), class = "Intercept"),
                              prior(normal(0, 5), class = "b"),
...
mod.brms.bites.8 = brm(formula =TotalBites ~ bite_on*habitat +site*habitat + (1|id/Date), 
                    data=dat.bites.sum, family="negbinomial", sample_prior = T,
                    # prior = m1priors,
                    prior = c(prior(normal(0, 5), class = "Intercept"),
                              prior(normal(0, 5), class = "b"),
...
mod.brms.bites.7ZI = brm(formula =TotalBites ~ bite_on*habitat +site*habitat + (1|Date), 
                    data=dat.bites.sum, family=zero_inflated_negbinomial(), sample_prior = T,
                    # prior = m1priors,
                    prior = c(prior(normal(0, 5), class = "Intercept"),
                              prior(normal(0, 5), class = "b"),
...
mod.brm.2 = add_criterion(mod.brms.bites.2,  "loo")
mod.brm.2.1 = add_criterion(mod.brms.bites.2.1,  "loo")
mod.brm.3 = add_criterion(mod.brms.bites.3,  "loo")
mod.brm.4 = add_criterion(mod.brms.bites.4,  "loo")
mod.brm.5 = add_criterion(mod.brms.bites.5,  "loo")
mod.brm.6 = add_criterion(mod.brms.bites.6,  "loo")
mod.brm.7 = add_criterion(mod.brms.bites.7,  "loo")
mod.brm.8 = add_criterion(mod.brms.bites.8,  "loo")
loo::loo_compare(mod.brm.2,mod.brm.2.1,  mod.brm.3,mod.brm.4, mod.brm.5, mod.brm.6, criterion = "loo")

emmeans::emmeans(mod.brms.bites.2,~ bite_on|habitat, type="response")
mod_embites2 = emmeans::emmeans(mod.brms.bites.2,~ bite_on|habitat, type="response")
mod_embites3 = emmeans::emmeans(mod.brms.bites.3,~bite_on|habitat, type="response")
mod_embites4 = emmeans::emmeans(mod.brms.bites.4,~bite_on|habitat, type="response")
mod_embites5 = emmeans::emmeans(mod.brms.bites.5,~bite_on|habitat, type="response")
mod_embites7 = emmeans::emmeans(mod.brms.bites.7,~bite_on|habitat, type="response")
multcomp::cld(mod_embites2)
multcomp::cld(mod_embites3)
multcomp::cld(mod_embites4)
multcomp::cld(mod_embites7)
p3a = dat.bites.sum %>%
  modelr::data_grid(bite_on, habitat) %>%
  add_fitted_draws(mod.brms.bites.2, re_formula = NULL,  n=1000)

p3b = mod.brms.bites.2 %>% emmeans(~ bite_on|habitat, type="response") %>%
  gather_emmeans_draws(type="response") %>%
  mutate(.value = exp(.value))  
...
p4b = mod.brms.bites.2 %>% emmeans(~ bite_on|habitat, type="response") %>%
  gather_emmeans_draws(type="response") %>%
  mutate(.value = exp(.value)) %>%
  compare_levels(.value, by = bite_on)
...
p3.1a = ggplot(p3a, aes(y = .value, x = bite_on, fill = bite_on)) +
  theme_bw(base_size = 12) + 
  coord_cartesian(ylim= c(0,400)) +
  geom_point(data=dat.bites.sum, aes(y=TotalBites, x=bite_on, colour=bite_on), 
             position = position_jitterdodge(jitter.width = 0.1),pch=1) +
  stat_eye(alpha=0.7, position = position_dodge(width = 0.7), .width = c(0.5, 0.95)) +
  # stat_gradientinterval(position = "dodge") +
  # scale_y_continuous(breaks = c(0.25, 0.5,0.75,1,1.25), limits = c(0.25,1.5)) +
  ...
  facet_grid(~habitat) +
  ylab("Proportion of progagules remaining")  
p3.1b = ggplot(p3b, aes(y = .value, x = bite_on, fill = bite_on)) +
  theme_bw(base_size = 12) + 
  coord_cartesian(ylim= c(0,400)) +
  geom_point(data=dat.bites.sum, aes(y=TotalBites, x=bite_on, colour=bite_on), 
             position = position_jitterdodge(jitter.width = 0.1),pch=1) +
...
dat.bites.spsum = dat.bites %>% 
  ...
  dplyr::mutate(TotalBites = replace_na(TotalBites, 0)) %>%
  filter(species %in% c("Ecsenius stictus", "Ctenochaetus striatus", "Ctenochaetis striatus"))
# 
...
pairs(emmeans::emmeans(mod.brms.All1,~ bite_on|species), reverse = T)
pairs(emmeans::emmeans(mod.brms.All2,~ bite_on|species), reverse = T)
...
```{r Species Specific Pooled}
#Non-Grazers
dat.bites.spmodCS =  dat.bites.spmod %>% 
  dplyr::filter(species %in% c("Ctenochaetus striatus", 'Ctenochaetis striatus')) %>% 
  mutate(TotalBites = round(TotalBites/3,0),
         species = "Ctenochaetus striatus")
dat.bites.spmodCS2 =  dat.bites.spmodCS %>% full_join(dplyr::select(dat.bites.sum, -TotalBites), 
                                                 by = c("Date", "id", "habitat", "bite_on")) %>%
  mutate(species  = "Ctenochaetus striatus", 
         TotalBites = replace_na(TotalBites, replace = 0)) %>%
  filter(habitat == "Crest")
dat.bites.spmodPO =  dat.bites.spmod %>% filter(species =="Pomacentrus")%>% 
  mutate(TotalBites = round(TotalBites/3,0))
dat.bites.spmodPO2 =  dat.bites.spmodPO %>% full_join(dplyr::select(dat.bites.sum, -TotalBites), 
                                                 by = c("Date", "id", "habitat", "bite_on")) %>%
  mutate(species = "Pomacentrus", 
         TotalBites = replace_na(TotalBites, replace = 0))
dat.bites.spmodES =  dat.bites.spmod %>% filter(species =="Ecsenius stictus")%>% 
  mutate(TotalBites = round(TotalBites/3,0))
dat.bites.spmodES2 =  dat.bites.spmodES %>%  full_join(dplyr::select(dat.bites.sum, -TotalBites), 
                                                 by = c("Date", "id", "habitat", "bite_on")) %>%
  mutate(species = "Ecsenius stictus", 
         TotalBites = replace_na(TotalBites, replace = 0))

# Pooled Grazers   
dat.bites.graz = dat.bites %>% 
  # filter(!is.na(bites)) %>%
  separate(species, into = c("Genus", "Species"), sep = " ") %>%
  group_by(Date, id, habitat, site, Genus, bite_on) %>%
  summarise(TotalBites = sum(bites, na.rm =T))%>%
  ...
  gather(bite_on, TotalBites, 6:7, na.rm = F) %>% arrange(Date, id, habitat, site, Genus) %>%
  dplyr::mutate(TotalBites = replace_na(TotalBites, 0)) %>%
  mutate(Grazer = ifelse(Genus %in% c("Scarus", "Acanthurus","Zebrasoma"), "Grazer", NA)) %>%
  filter(Grazer =="Grazer") %>%
  group_by(Date, id, habitat, site, bite_on) %>%
  summarise(TotalBites = sum(TotalBites, na.rm =T)) %>% 
  mutate(TotalBites = round(TotalBites/3,0))

# Pooled Grazers   
dat.bites.scarus = dat.bites %>% 
  # filter(!is.na(bites)) %>%
  separate(species, into = c("Genus", "Species"), sep = " ") %>%
  group_by(Date, id, habitat, site, Genus, bite_on) %>%
  summarise(TotalBites = sum(bites, na.rm =T))%>%
  ...
  gather(bite_on, TotalBites, 6:7, na.rm = F) %>% arrange(Date, id, habitat, site, Genus) %>%
  dplyr::mutate(TotalBites = replace_na(TotalBites, 0)) %>%
  mutate(Grazer = ifelse(Genus %in% c("Scarus"), "Grazer", NA)) %>%
  filter(Grazer =="Grazer") %>%
  group_by(Date, id, habitat, site, bite_on) %>%
  summarise(TotalBites = sum(TotalBites, na.rm =T)) %>% 
  mutate(TotalBites = round(TotalBites/3,0))

dat.bites.graz2 =  dat.bites.graz %>% full_join(dplyr::select(dat.bites.sum, -TotalBites), 
                                                 by = c("Date", "id", "habitat", "bite_on")) %>%
  mutate(species = "Grazers", 
         TotalBites = replace_na(TotalBites, replace = 0)) %>%
  filter(habitat == "Crest")
dat.bites.scarus2 =  dat.bites.scarus %>% full_join(dplyr::select(dat.bites.sum, -TotalBites), 
                                                 by = c("Date", "id", "habitat", "bite_on")) %>%
  mutate(species = "Grazers", 
         TotalBites = replace_na(TotalBites, replace = 0)) %>%
  filter(habitat == "Crest")
# Fit Models
mpriors = brms::get_prior(formula =TotalBites ~ bite_on*habitat + (1|id/Date), 
                    data=dat.bites.spmodPO2, family="negbinomial")
mod.brms.PO = brm(formula =TotalBites ~ bite_on*habitat + (1|id/Date), 
                    data=dat.bites.spmodPO2, family="negbinomial", sample_prior = T,
                    prior = mpriors,
                    # prior = c(prior(normal(0, 5), class = "Intercept"),
                    #           prior(normal(0, 5), class = "b"),
                    ...
                    warmup = 200, iter = 2000, chains = 3, cores = 3, thin=2,
                    control = list(adapt_delta = 0.98))
mpriors = brms::get_prior(formula =TotalBites ~ bite_on + (1|id/Date), 
                    data=dat.bites.spmodCS2, family="negbinomial")
mod.brms.CS = brm(formula =TotalBites ~ bite_on+  (1|id/Date), 
                    data=dat.bites.spmodCS2, family="negbinomial", sample_prior = T,
                  prior = mpriors,
                    # prior = c(prior(normal(0, 5), class = "Intercept"),
                    #           prior(normal(0, 5), class = "b"),
                    ...
                    control = list(adapt_delta = 0.98))
mpriors = brms::get_prior(formula =TotalBites ~ bite_on*habitat + (1|id/Date), 
                    data=dat.bites.spmodES2, family="negbinomial")
mod.brms.ES = brm(formula =TotalBites ~ bite_on*habitat + + (1|id/Date), 
                    data=dat.bites.spmodES2, family="negbinomial", sample_prior = T,
                    prior =mpriors,
                    # prior = c(prior(normal(0, 5), class = "Intercept"),
                    #           prior(normal(0, 5), class = "b"),
                    ...
                    control = list(adapt_delta = 0.98))
mpriors = brms::get_prior(formula =TotalBites ~ bite_on + (1|id/Date), 
                    data=dat.bites.graz2, family="negbinomial")
mod.brms.GZ = brm(formula =TotalBites ~ bite_on+  (1|id/Date), 
                    data=dat.bites.graz2, family="negbinomial", sample_prior = T,
                    prior = mpriors,
                    warmup = 200, iter = 2000, chains = 3, cores = 3, thin=2,
                    control = list(adapt_delta = 0.98))
mod.brms.SC = brm(formula =TotalBites ~ bite_on+ + (1|id/Date), 
                    data=dat.bites.scarus2, family="negbinomial", sample_prior = T,
                    prior = c(prior(normal(0, 5), class = "Intercept"),
                              prior(normal(0, 5), class = "b"),
                              prior(normal(0,5), class = "sd"), 
                              prior(gamma(0.1, 0.1), class = "shape")),
                    warmup = 200, iter = 2000, chains = 3, cores = 3, thin=2,
                    control = list(adapt_delta = 0.98))
# mod.brms.GZZI = brm(formula =TotalBites ~ bite_on+ (1|id/Date), 
#                     data=dat.bites.graz2, family=zero_inflated_negbinomial(), sample_prior = T,
#                     prior = c(prior(normal(0, 5), class = "Intercept"),
#                               prior(normal(0, 5), class = "b"),
#                               prior(normal(0,5), class = "sd"), 
#                               prior(gamma(0.1, 0.1), class = "shape")),
...
#                     control = list(adapt_delta = 0.98))
# 
# mod.brms.GZno0 = brm(formula =TotalBites ~ bite_on + (1|id/Date), 
#                     data=dat.bites.graz, family="negbinomial", sample_prior = T,
#                     prior = c(prior(normal(0, 5), class = "Intercept"),
#                               prior(normal(0, 5), class = "b"),
#                               prior(normal(0,5), class = "sd"), 
#                               prior(gamma(0.1, 0.1), class = "shape")),
#                     warmup = 200, iter = 2000, chains = 3, cores = 3, thin=2,
#                     control = list(adapt_delta = 0.98))
mod.brm.gz = add_criterion(mod.brms.GZ,  "loo")
# mod.brm.gzZI = add_criterion(mod.brms.GZZI,  "loo")
# loo::loo_compare(mod.brm.gz,  mod.brm.gzZI, criterion = "loo")

mod_emGZ = emmeans::emmeans(mod.brms.GZ,~ bite_on, type="response")
# mod_emGZ2 = emmeans::emmeans(mod.brms.GZZI,~ bite_on, type="response")
# mod_emGZ3 = emmeans::emmeans(mod.brms.GZno0,~ bite_on, type="response")

pairs(mod_emGZ, reverse = T)
# pairs(emmeans::emmeans(mod.brms.GZZI,~ bite_on, type="response"), reverse = T)
# pairs(emmeans::emmeans(mod.brms.GZno0,~ bite_on, type="response"), reverse = T)

#USE MODEL MOD.BRMS.gz

# Fit without adding zeros back in
mpriors = brms::get_prior(formula =TotalBites ~ bite_on*habitat  + (1|id/Date), 
                    data=dat.bites.spmodPO, family="negbinomial")
mod.brms.PO.2 = brm(formula =TotalBites ~ bite_on*habitat + (1|id/Date),
                    ...
                    control = list(adapt_delta = 0.98))
mpriors = brms::get_prior(formula =TotalBites ~ bite_on + (1|id/Date), 
                    data=dat.bites.spmodCS, family="negbinomial")
mod.brms.CS.2 = brm(formula =TotalBites ~ bite_on + (1|id/Date), 
                    ...
                    control = list(adapt_delta = 0.98))
mpriors = brms::get_prior(formula =TotalBites ~ bite_on*habitat  + (1|id/Date), 
                    data=dat.bites.spmodES, family="negbinomial")
mod.brms.ES.2 = brm(formula =TotalBites ~ bite_on*habitat + (1|id/Date), 
                    data=dat.bites.spmodES, family="negbinomial", sample_prior = T,
                    prior = mpriors,
                    warmup = 200, iter = 2000, chains = 3, cores = 3, thin=2,
                    control = list(adapt_delta = 0.98))
mpriors = brms::get_prior(formula =TotalBites ~ bite_on + (1|id/Date), 
                    data=dat.bites.graz, family="negbinomial")
mod.brms.GZ.2 = brm(formula =TotalBites ~ bite_on+ (1|id/Date), 
                    data=dat.bites.graz, family="negbinomial", sample_prior = T,
                    prior = mpriors,
                    warmup = 400, iter = 4000, chains = 3, cores = 3, thin=2,
                    control = list(adapt_delta = 0.98))
mpriors = brms::get_prior(formula =TotalBites ~ bite_on + (1|id/Date), 
                    data=dat.bites.scarus, family="negbinomial")
mod.brms.SC.2 = brm(formula =TotalBites ~ bite_on+ (1|id/Date), 
                    data=dat.bites.scarus, family="negbinomial", sample_prior = T,
                    prior = mpriors,
                    warmup = 400, iter = 4000, chains = 3, cores = 3, thin=2,
                    control = list(adapt_delta = 0.98))
mod_emGZ.2 = emmeans::emmeans(mod.brms.GZ.2,~ bite_on, type="response")
pairs(mod_emGZ.2, reverse=T)

mod.GRZ.2 = add_criterion(mod.brms.GZ.2,  "loo")
mod.GRZ.22 = add_criterion(mod.brms.GZ.22,  "loo")
loo::loo_compare(mod.GRZ.2,mod.GRZ.22,  criterion = "loo")
...
pairs(emmeans::emmeans(mod.brms.PO.2,~ bite_on|habitat, type="response", level=0.95), reverse = T)
...
pairs(emmeans::emmeans(mod.brms.CS.2,~ bite_on, type="response",level=0.95), reverse = T)
```

```{r Plot pooled species results}
...
#Start HEre for remaining species !!!!
...
```{r Fit full species models}
dat.bites.mod = bind_rows(dat.bites.spmodES2, dat.bites.spmodPO2)
dat.bites.mod2 = bind_rows(dat.bites.spmodAC2,  dat.bites.spmodSC2,dat.bites.spmodCS2)
mod.brms.All1 = brm(formula =TotalBites ~ bite_on*species +habitat + (1|site) +(1|Date), 
...
mod.brms.All1_ZI = brm(formula =TotalBites ~ bite_on*species +habitat + (1|site) +(1|id/Date), 
                    ...
                    control = list(adapt_delta = 0.98))
mod.brms.All1_ZI.2 = brm(formula =TotalBites ~ bite_on*species +habitat +(1|Date), 
                    data=dat.bites.mod, family=zero_inflated_negbinomial(), sample_prior = T,
                    prior = c(prior(normal(0, 5), class = "Intercept"),
                              prior(normal(0, 5), class = "b"),
                              prior(normal(0,5), class = "sd"), 
                    ...
                    warmup = 300, iter = 3000, chains = 3, cores = 3, thin=2,
                    control = list(adapt_delta = 0.98))
mod.brms.All1_ZI.3 = brm(formula =TotalBites ~ bite_on*species +habitat +(1|id/Date), 
                    data=dat.bites.mod, family=zero_inflated_negbinomial(), sample_prior = T,
                    prior = c(prior(normal(0, 5), class = "Intercept"),
                              prior(normal(0, 5), class = "b"),
                              prior(normal(0,5), class = "sd"), 
                    ...
                    warmup = 300, iter = 3000, chains = 3, cores = 3, thin=2,
                    control = list(adapt_delta = 0.98))
mod.brms.All1_ZI.4 = brm(formula =TotalBites ~ bite_on*species +habitat +(1|id/Date) +(1|site), 
                    data=dat.bites.mod, family=zero_inflated_negbinomial(), sample_prior = T,
                    prior = c(prior(normal(0, 5), class = "Intercept"),
                              prior(normal(0, 5), class = "b"),
                              prior(normal(0,5), class = "sd"), 
                    ...
                    warmup = 300, iter = 3000, chains = 3, cores = 3, thin=2,
                    control = list(adapt_delta = 0.98))
mod.brms.All2 = brm(formula =TotalBites ~ bite_on*species +  (1|site) + (1|Date), 
                    data=dat.bites.mod2, family="negbinomial", sample_prior = T,
                    prior = c(prior(normal(0, 5), class = "Intercept"),
                              prior(normal(0, 5), class = "b"),
                              prior(normal(0,5), class = "sd"), 
                              prior(gamma(0.1, 0.1), class = "shape")),
...
mod.brms.All2_ZI.3 = brm(formula =TotalBites ~ bite_on*species + (1|id/Date), 
                    ...
                    warmup = 200, iter = 2000, chains = 3, cores = 3, thin=2,
                    control = list(adapt_delta = 0.98))
waic1 = waic(mod.brms.All1)
waic1_ZI = waic(mod.brms.All1_ZI)
waic2 = waic(mod.brms.All2)
waic2_ZI = waic(mod.brms.All2_ZI)
loo1 = loo(mod.brms.All1)
loo1_ZI = loo(mod.brms.All1_ZI)
loo2 = loo(mod.brms.All2)
loo2_ZI = loo(mod.brms.All2_ZI)
loo1_ZI.2 = loo(mod.brms.All1_ZI.2)
loo1_ZI.3 = loo(mod.brms.All1_ZI.3)
loo1_ZI.4 = loo(mod.brms.All1_ZI.4)
which(loo2_ZI$diagnostics$pareto_k >0.7)
which(loo2$diagnostics$pareto_k >0.7)
which(loo1$diagnostics$pareto_k >0.7)
which(loo1_ZI$diagnostics$pareto_k >0.7)
compare_ic(waic1, waic1_ZI)
compare_ic(waic2, waic2_ZI)
